...
void draw() {
    ...
    if (stopper == 0) {
        ...
        switch (letter) {
            case "A":
                lowerjaw = -QUARTER_PI;
                upperjaw = PI + QUARTER_PI; // MOVING UP (ANIMATION + DETECTION OF COLOR)
                c = get(pacx, pacy - 18);
                d = get(pacx + 12, pacy - 18);
                f = get(pacx - 12, pacy - 18);
                break;
            case "B":
                lowerjaw = -PI - QUARTER_PI;
                upperjaw = QUARTER_PI; // MOVING DOWN (ANIMATION + DETECTION OF COLOR)
                c = get(pacx, pacy + 18);
                d = get(pacx + 12, pacy + 18);
                f = get(pacx - 12, pacy + 18);
                break;
            case "C":
                lowerjaw = -HALF_PI - QUARTER_PI;
                upperjaw = PI - QUARTER_PI; // MOVING LEFT (ANIMATION + DETECTION OF COLOR)
                c = get(pacx - 18, pacy);
            ...
            case "D":
                lowerjaw = QUARTER_PI;
                upperjaw = PI + HALF_PI + QUARTER_PI; // MOVING RIGHT (ANIMATION + DETECTION OF COLOR)
                c = get(pacx + 18, pacy);
                d = get(pacx + 18, pacy + 12);
                f = get(pacx + 18, pacy - 12);
                break;
        ...
        // red ghost moving
        if (ghostrY <= 30 || ghostrY >= 430) {
            ghostrspeed = ghostrspeed * -1;
        }
        ghostrY = ghostrY + ghostrspeed;
        // blue ghost moving
        if (ghostbX <= 179 || ghostbX >= 335) {
            ghostbspeed = ghostbspeed * -1;
        }
        ghostbX = ghostbX + ghostbspeed;
        // orange ghost moving
        if (ghostoX <= 35 || ghostoX >= 470) {
            ghostospeed = ghostospeed * -1;
        }
        ghostoX = ghostoX + ghostospeed;
        //pink ghost moving
        if (ghostpX <= 39 || ghostpX >= 230) {
            ghostpspeed = ghostpspeed * -1;
        }
        ghostpX = ghostpX + ghostpspeed;
        // yellow ghost moving
        if (ghostyY <= 30 || ghostyY >= 430) {
            ghostyspeed = ghostyspeed * -1;
        }
        ghostyY = ghostyY + ghostyspeed;
        //grey ghost moving
        if (ghostgX <= 120 || ghostgX >= 390) {
            ghostgspeed = ghostgspeed * -1;
        }
        ghostgX = ghostgX + ghostgspeed;
        //turqoise ghost moving
        if (ghosttY <= 30 || ghosttY >= 140) {
            ghosttspeed = ghosttspeed * -1;
        }
